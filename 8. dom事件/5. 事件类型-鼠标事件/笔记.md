...
- over和out **不考虑子元素，从父元素移动到子元素，对于父元素而言，仍然算作离开,会触发事件**
- enter和leave，考虑子元素，子元素仍然是父元素的一部分
- mouseenter和mouseleave不会冒泡 

**细节：给元素绑定事件时，拖动太快，导致监听不到，可以给整个window绑定（原理事件冒泡）**
...
- movement：movementX、movementY，只在鼠标移动事件中有效，相对于上一次鼠标位置，偏移的距离 **触发频率和浏览器版本和电脑配置有关，在mac和windows上不同**